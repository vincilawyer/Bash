...
$(pz "SPEEDTEST_all")                              #@测速范围#@对所有IP (仅支持 IPv4) 进行测速，默认每个 /24 段随机测速一个 IP，输入all测试全部,输入＃默认范围#@\"[[ \"\$new_text\" == \"y\" ]]\"
...
function start_speedtest {
    cd "$path_CFST_file"
   echo "当前测速配置为 "$([[ -n "$SPEEDTEST_URL" ]] && echo "-url "$SPEEDTEST_URL"")" "$([[ -n "$SPEEDTEST_n" ]] && echo "-n "$SPEEDTEST_n"")" \
