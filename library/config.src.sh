...
# 第一个#@后为：变量名称
# 第一个#@后为：备注
# 第三个#@后为：匹配规则（有条件规则和比较规则）。1、比较规则：即为正则表达式的变量名（请注意，不是正则表达本身，需要在arg中定义正则表达式），输入结果将与该正则表达式作比较。2、条件规则：为判断new_text变量是否符合规则条件
# 条件规则需用两个双引号包裹，并且条件规则中所有的双引号、\$号等都需要用\号前置，防止被转义。
